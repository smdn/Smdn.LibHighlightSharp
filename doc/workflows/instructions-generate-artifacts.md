...
The instructions in this document generates artifacts for Highlight version **4.6**, and references **4.5** as the previous version.
...
```
git switch -c artifact-highlight-4.6
```
...
git commit -m "bump build target version up to highlight-4.6" -- src/Highlight.Build.props
```
...
diff highlight-4.5/ highlight-4.6/
cd -
```
...
```
git add src/Smdn.LibHighlightSharp.Bindings/Smdn.LibHighlightSharp.Bindings/highlight-4.6/

git commit -m "add bindings for highlight-4.6" -- src/Smdn.LibHighlightSharp.Bindings/Smdn.LibHighlightSharp.Bindings/highlight-4.6/
```

### Push the working branch
```
git push github artifact-highlight-4.6
```
...
```
git pull github artifact-highlight-4.6
```

Create a PR to merge the changes up to this point into the `main` branch.

```
gh pr create --base main --head artifact-highlight-4.6 --assignee @me --title "Add artifact highlight-4.6" --fill
```
...
git branch -d artifact-highlight-4.6
```
...
```
git commit -m "add CompatibilitySuppressions entries for highlight >=4.6" -- src/Smdn.LibHighlightSharp.Bindings/CompatibilitySuppressions.xml
```
...
```
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.LangDefs-4.6.0
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.Themes-4.6.0
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.Bindings-4.6.0

git push github main --tags

git tag -d new-release/main/Smdn.LibHighlightSharp.LangDefs-4.6.0
git tag -d new-release/main/Smdn.LibHighlightSharp.Themes-4.6.0
git tag -d new-release/main/Smdn.LibHighlightSharp.Bindings-4.6.0
```
