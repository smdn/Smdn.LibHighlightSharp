...
The instructions in this document generates artifacts for Highlight version **4.7**, and references **4.6** as the previous version.
...
```
git switch -c artifact-highlight-4.7
```
...
```
sed -i -e 's/Highlight_SourceVersionMajorMinor>4\.6/Highlight_SourceVersionMajorMinor>4\.7/g' src/Highlight.Build.props

git commit -m "bump build target version up to highlight-4.7" -- src/Highlight.Build.props
```
...
diff highlight-4.6/ highlight-4.7/
cd -
```
...
```
git add src/Smdn.LibHighlightSharp.Bindings/Smdn.LibHighlightSharp.Bindings/highlight-4.7/

git commit -m "add bindings for highlight-4.7" -- src/Smdn.LibHighlightSharp.Bindings/Smdn.LibHighlightSharp.Bindings/highlight-4.7/
```

### Push the working branch
```
git push github artifact-highlight-4.7
```
...
```
git pull github artifact-highlight-4.7
```

Create a PR to merge the changes up to this point into the `main` branch.

```
gh pr create --base main --head artifact-highlight-4.7 --assignee @me --title "Add artifact highlight-4.7" --fill
```
...
git branch -d artifact-highlight-4.7
```
...
```
git commit -m "add CompatibilitySuppressions entries for highlight >=4.7" -- src/Smdn.LibHighlightSharp.Bindings/CompatibilitySuppressions.xml
```
...
```
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.LangDefs-4.7.0
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.Themes-4.7.0
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.Bindings-4.7.0

git push github main --tags

git tag -d new-release/main/Smdn.LibHighlightSharp.LangDefs-4.7.0
git tag -d new-release/main/Smdn.LibHighlightSharp.Themes-4.7.0
git tag -d new-release/main/Smdn.LibHighlightSharp.Bindings-4.7.0
```
