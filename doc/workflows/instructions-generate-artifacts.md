...
The instructions in this document generates artifacts for Highlight version **4.22**, and references **4.21** as the previous version.
...
```
git switch -c artifact-highlight-4.22
```
...
```
sed -i -e 's/Highlight_SourceVersionMajorMinor>4\.21/Highlight_SourceVersionMajorMinor>4\.22/g' src/Highlight.Build.props

git commit -m "bump build target version up to highlight-4.22" -- src/Highlight.Build.props
```
...
diff highlight-4.21/ highlight-4.22/
cd -
```
...
```
git add src/Smdn.LibHighlightSharp.Bindings/Smdn.LibHighlightSharp.Bindings/highlight-4.22/
git commit -m "add bindings for highlight-4.22" -- src/Smdn.LibHighlightSharp.Bindings/Smdn.LibHighlightSharp.Bindings/highlight-4.22/
```

### Push the working branch
```
git push github artifact-highlight-4.22
```
...
```
git pull github artifact-highlight-4.22
```

Create a PR to merge the changes up to this point into the `main` branch.

```
gh pr create --base main --head artifact-highlight-4.22 --assignee @me --title "Add artifact highlight-4.22" --fill
```
...
git branch -d artifact-highlight-4.22
```
...
```
dotnet pack /p:GenerateCompatibilitySuppressionFile=true src/Smdn.LibHighlightSharp.Bindings/ && git restore doc/api-list
...
```
git commit -m "add CompatibilitySuppressions entries for highlight >=4.22" -- src/Smdn.LibHighlightSharp.Bindings/CompatibilitySuppressions.xml
```
...
```
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.LangDefs-4.22.0
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.Themes-4.22.0
git tag -m "new release" new-release/main/Smdn.LibHighlightSharp.Bindings-4.22.0

git push github \
  new-release/main/Smdn.LibHighlightSharp.LangDefs-4.22.0 \
  new-release/main/Smdn.LibHighlightSharp.Themes-4.22.0 \
  new-release/main/Smdn.LibHighlightSharp.Bindings-4.22.0
